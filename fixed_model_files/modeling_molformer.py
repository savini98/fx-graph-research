...
class MolformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            # GraphMend Predicated Guard transformation (GB0033):
            # Step 1 — shape/dtype check: Python-level bools, Dynamo guards on them statically
            #           (no graph break). Short-circuits to tensor(False) if shapes/dtypes differ,
            #           avoiding a broadcasting error from (a == b).all() on incompatible shapes.
            # Step 2 — value check: (a == b).all() stays as a tensor op inside the FX graph.
            # Step 3 — torch._assert_async: graph-native assertion, fires at runtime if False.
            _shapes_dtypes_ok = (
                attention_mask.shape == per_query_extended.shape
                and attention_mask.dtype == per_query_extended.dtype
            )
            torch._assert_async(
                (attention_mask == per_query_extended).all()
                if _shapes_dtypes_ok
