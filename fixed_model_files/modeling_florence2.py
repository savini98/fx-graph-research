...
class Florence2EncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # dtype check becomes a static Dynamo guard; tensor-dependent condition
        # replaced with | (tensor op) and torch.where (no graph break)
        if hidden_states.dtype == torch.float16:
            clamp_value = torch.finfo(hidden_states.dtype).max - 1000
            needs_clamp = torch.isinf(hidden_states).any() | torch.isnan(hidden_states).any()
