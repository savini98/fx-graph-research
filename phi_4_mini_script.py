...
def fixed_batch(tok, bs=1, seq_len=5):
    prompt = "Write a poem about AI"
    enc = tok(prompt, return_tensors="pt", padding="max_length", truncation=True, max_length=seq_len)
...
def main():
    ...
    trace_path = f"traces/phi4mini_trace_{TYPE}_{dt_str}.json"
    ...
    # Save to CSV
    csv_filename = f"traces/phi_4_mini_instruct_benchmark_runs_{TYPE}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
