...
# load longformer-scico model files from Hugging Face repository
if [ -d "longformer-scico" ]; then
    echo "Removing existing flan-t5-large directory..."
    rm -rf longformer-scico
fi

echo "cloning repository"
git clone https://huggingface.co/allenai/longformer-scico

echo "Verifying LFS files for longformer-scico model..."
cd longformer-scico
git lfs fsck
git lfs pull
cd ..

# load biogpt files from Hugging Face repository
if [ -d "biogpt" ]; then
    echo "Removing existing biogpt directory..."
    rm -rf biogpt
fi
...
echo "Verifying LFS files for biogpt model..."
cd biogpt
git lfs fsck
git lfs pull
cd ..
