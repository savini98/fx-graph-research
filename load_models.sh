...
# load Phi-mini-MoE-instruct model files from Hugging Face repository
if [ -d "Phi-mini-MoE-instruct" ]; then
    echo "Removing existing Phi-mini-MoE-instruct directory..."
    rm -rf Phi-mini-MoE-instruct
fi

echo "cloning repository"
git clone https://huggingface.co/microsoft/Phi-mini-MoE-instruct

echo "Verifying LFS files for Phi-mini-MoE-instruct model..."
cd Phi-mini-MoE-instruct
git lfs fsck
git lfs pull
cd ..