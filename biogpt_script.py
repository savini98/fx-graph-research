...
def generate_text(
    ...
    # Remove decoder_input_ids if present (for causal LM)
    batch = {k: v for k, v in batch.items() if k != 'decoder_input_ids'}
    # Only set max_length if max_new_tokens is not present
    if 'max_new_tokens' not in gen_kwargs and 'max_length' not in gen_kwargs:
        gen_kwargs['max_length'] = 128  # or another reasonable default
    ...
    new_tokens = max(0, total_len - input_len)+30
    runtime_s = end - start
