...
MODEL_ID = "tiny-random-PegasusForCausalLM"
...
def main():
    ...
    if args.batch_size <= 0:
        ...
        BATCH_SIZE = find_max_batch_size(
            ...
            model_fn=lambda **b: model(**{k: v for k, v in b.items() if k in ("input_ids", "attention_mask")}),
        )
    ...
    # Remove decoder_input_ids and decoder_inputs_embeds if present to avoid TypeError for decoder-only models
    batch_for_forward = {k: v for k, v in batch.items() if k in ["input_ids", "attention_mask"]}
    with torch.inference_mode():
        _ = model(**batch_for_forward)
        torch.cuda.synchronize()
    t("eager ok")
