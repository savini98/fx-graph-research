...
MODEL_ID = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bart-base")
OUTPUT_TOKEN_LENGTH = 100  # Set your desired output token length limit here
PROMPT = "Summarize the following article: Artificial intelligence has transformed many industries including healthcare, finance, and transportation. Recent advances in large language models have enabled new applications in text generation, translation, and summarization."
...
def fixed_batch(tok, bs=1, seq_len=5):
    ...
    # Use BOS/pad token as start for decoder_input_ids
...
def generate_text(
    ...
    # For generation, pass only input_ids and attention_mask (not decoder_input_ids)
    gen_batch = {k: v for k, v in batch.items() if k != 'decoder_input_ids'}
    out = model.generate(**gen_batch, **gen_kwargs)
    ...
    new_tokens = total_len  # For Seq2Seq, output is decoder-only tokens
    runtime_s = end - start
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=0, help='Batch size (0=auto-detect max for GPU)')
    ...
    # Auto-detect max batch size if not specified
