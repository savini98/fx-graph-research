...
class QWenModel(QWenPreTrainedModel):
    ...
    def _normalize_audio_info(audio_info):
        # Normalize `audio_info` to tensors/lists expected by self.audio.encode
        if isinstance(audio_info, dict):
            ...
            audios = torch.concat([_["input_audios"] for _ in audio_info], dim=0)
            input_audio_lengths = torch.concat([_["input_audio_lengths"] for _ in audio_info], dim=0)
            audio_span_tokens = []
            for _ in audio_info:
                audio_span_tokens.extend(_["audio_span_tokens"])
        return audios, input_audio_lengths, audio_span_tokens
